...
# This repo carries a `book/` directory, and `book` is a task. The catch-all below has
# `$(UVX)` as its only prerequisite, so make compares mtimes and prints "`book' is up to
# date." whenever the directory is newer than the uvx binary -- the book silently stops
# building instead of failing. `.PHONY` can name a target the catch-all would otherwise
# resolve. It needs the forwarding recipe spelled out alongside it: `.PHONY` takes `book`
# out of the catch-all's reach, and a phony target with no recipe is a silent no-op, so the
# declaration on its own trades "is up to date" for "Nothing to be done".
.PHONY: book
book: $(UVX)
	@$(UVX) $(RHIZA_TASK) book
